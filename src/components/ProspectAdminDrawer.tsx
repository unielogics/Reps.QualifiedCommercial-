...
export default function ProspectAdminDrawer({
  ...
  const updateAccess = useMutation({
    mutationFn: async ({ user, enabled }: { user: ProspectAccessUser; enabled: boolean }) => api<ProspectAccessUser>(`/dealer-os/admin/prospect-access/${user.user_id}`, {
      method: "PATCH",
      body: JSON.stringify({ enabled, reason: enabled ? "Enabled from Marketing pipeline configuration" : "Disabled from Marketing pipeline configuration" }),
      authToken: (await getToken()) ?? undefined,
    }),
    onSuccess: async () => {
  ...
  return <Drawer title="Configure Marketing pipeline" width={1050} onClose={onClose} variant="workspace">
...
function accessEligibility(user: ProspectAccessUser): string {
  ...
  return "This role is not eligible for the Marketing pipeline";
}
...
function ProspectAccessEditor({ data, loading, updatingUserId, onToggle }: { data?: ProspectAccessList; loading: boolean; updatingUserId?: string; onToggle: (user: ProspectAccessUser, enabled: boolean) => void }) {
  ...
  return <div className="prospectAccessAdmin">
    <div className="prospectAccessIntro"><span><Users size={21} /></span><div><h3>Marketing pipeline access</h3><p className="sub">Enable the outreach package only for agents participating in the rollout. Removing access hides the pipeline and blocks new outreach without deleting their historical Marketing email log.</p></div><div><b>{liveCount}</b><small>live user{liveCount === 1 ? "" : "s"}</small></div></div>
