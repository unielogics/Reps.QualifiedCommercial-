...
type Locale = "en" | "es";
type Product = { program_key: string; category: string; name: string; summary: string | null; highlights: string[]; pricing: string | null; disclosure: string | null; amount_min: number | null; amount_max: number | null; term_min_months: number | null; term_max_months: number | null; direct_action: "start_application" | "book_call"; icon_key: string };
...
export default function ProductsPage() {
  ...
  const createSession = useMutation({
    ...
    onSuccess: (data) => { setSession(data); setAnswers((old) => ({ ...(data.answers as Record<string, string | number | boolean>), ...old })); setMessage(""); },
  });
  ...
  return <div className="productsPage">
    ...
    {mode === "browse" ? <>
      ...
      <div className="productRail" ref={rail}>{items.map((item) => { const on = selected.includes(item.program_key); return <article className={`productCard ${on ? "selected" : ""}`} key={item.program_key} onClick={() => router.push(`/products/${item.program_key}?locale=${locale}`)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") router.push(`/products/${item.program_key}?locale=${locale}`); }}>
        ...
        <span className="productOpenHint">{t.open} <ChevronRight size={15} /></span>
        <div className="productActions" onClick={(event) => event.stopPropagation()}><button className={`btn ${on ? "pri" : ""}`} onClick={() => setSelected((old) => on ? old.filter((key) => key !== item.program_key) : [...old, item.program_key])}>{t.compare}</button>{item.direct_action === "start_application" ? <button className="btn" onClick={() => router.push("/?new=1")}>{t.start}</button> : <button className="btn" disabled={!booking.data?.enabled} onClick={() => booking.data?.url && window.open(booking.data.url, "_blank", "noopener,noreferrer")}><CalendarDays size={16} /> {t.book}</button>}</div>
      </article>; })}</div>
      ...
      <div className="finderMain">
        ...
        : <div className="finderConversation"><div className="finderFact"><b>{profile.company_name}</b><span>{money(Number(profile.requested_amount))} · {profile.use_of_funds}</span></div>
          ...
          {result && <div className="screenResults"><div className="screenSummary"><div><span>{t.requested}</span><b>{money(result.client_requested_amount)}</b></div><div><span>{t.recommended}</span><b>{money(result.recommended_amount)}</b></div><small>{result.verification}</small></div>{result.evaluated_programs.map((program) => <article key={program.program_key} className={`screenProgram ${program.status}`}><div><span className={`cellchip ${program.status === "recommended" ? "c-ok" : program.status === "blocked" ? "c-bad" : "c-warn"}`}>{program.status}</span><h4>{program.name}</h4></div>{program.strengths.map((item) => <p className="screenGood" key={item}>✓ {item}</p>)}{program.unresolved.map((item) => <p key={item}>• {item}</p>)}{program.borrower_safe_reasons.map((item) => <p className="screenBad" key={item}>× {item}</p>)}</article>)}
            <div className="screenActions">{result.amount_adjustment_required && <button className="btn" onClick={() => confirmGoal.mutate()}>{t.confirm} · {money(result.recommended_amount)}</button>}{result.evaluated_programs.some((program) => program.status !== "blocked") ? <button className="btn pri" onClick={() => startApplication.mutate()}>{t.start}</button> : <button className="btn pri" disabled={!booking.data?.enabled} onClick={() => booking.data?.url && window.open(booking.data.url, "_blank", "noopener,noreferrer")}><CalendarDays size={16} /> {t.book}</button>}</div></div>}
        </div>}
      </div>
    </section>}
