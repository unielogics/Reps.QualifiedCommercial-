...
export default function FieldDeskSettingsPage() {
  ...
  const save = useMutation<Profile, Error, ProfileForm>({
    ...
    // The phone writes the user row too (it lifts the first-login gate and prints on agreements), so /auth/me changes with it.
    onSuccess: async () => { setMessage("Profile saved. New business-card shares will use these details."); await queryClient.invalidateQueries({ queryKey: ["field-desk-profile"] }); await queryClient.invalidateQueries({ queryKey: ["me"] }); },
  ...
  return <div className="settingsPage">
    ...
    {isSuperAdmin && <a className="settingsFormsCallout mt" href="/settings/forms"><span><Files size={22} /></span><span><b>Forms and Packages</b><small>Upload immutable PDF versions, map them to EZ Term or MicroCap, order supporting agreements, and publish what Step 4 fills and sends.</small></span><span>Open configuration →</span></a>}
    <div className="settingsLayout mt">
      <section className="panel settingsFormPanel"><div className="panelTitle"><div><h3>Business-card profile</h3><span>Visible only when you share your card.</span></div></div><div className="settingsForm">
        ...
        <div className="fieldGrid"><label><span>Display name</span><input className="field" value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} /></label><label><span>Title</span><input className="field" placeholder="Commercial Funding Advisor" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><label><span>Display email</span><input className="field" type="email" value={form.display_email} onChange={(event) => setForm({ ...form, display_email: event.target.value })} /></label><label><span>Phone</span><input className="field" inputMode="tel" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /><small>Also the number printed as the relationship manager&apos;s phone on production agreements.</small></label><label><span>Preferred card language</span><select className="field" value={form.preferred_locale} onChange={(event) => setForm({ ...form, preferred_locale: event.target.value as "en" | "es" })}><option value="en">English</option><option value="es">Español</option></select></label><label className="settingsToggle"><span><b>Public card enabled</b><small>Allow active shared links to show your current corrections.</small></span><input type="checkbox" checked={form.card_visible} onChange={(event) => setForm({ ...form, card_visible: event.target.checked })} /></label><label className="full"><span>Short bio</span><textarea className="field" rows={5} value={form.short_bio} onChange={(event) => setForm({ ...form, short_bio: event.target.value })} placeholder="A concise introduction for clients receiving your card." /></label></div>
