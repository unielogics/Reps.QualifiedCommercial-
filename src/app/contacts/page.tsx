...
import { Columns3, GripVertical, List, Mail, Plus, Settings2, Trash2 } from "lucide-react";
...
import ContactArchiveDialog from "@/components/ContactArchiveDialog";

type Contact = { id: string; name: string; company: string | null; email: string | null; phone: string | null; source: string; updated_at: string; can_archive?: boolean };
...
export default function MarketingPage() {
  ...
  return <div className="contactsPage prospectPipelinePage">
    ...
    {section === "contacts" && <div className="panel mt"><div className="tblwrap"><table className="tbl portfolioTable contactTable"><thead><tr><th>Contact</th><th>Company</th><th>Email</th><th>Mobile</th><th>Source</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
      {contactRows.map((contact) => { const href = marketingDetailHref(`/marketing/${contact.id}`, returnTo); const open = () => { rememberContext(); router.push(href); }; return <tr key={contact.id} role="link" tabIndex={0} aria-label={`Open ${contact.name}`} onClick={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }}><td><span className="contactTableIdentity"><span className="contactAvatar compact">{initials(contact.name)}</span><b>{contact.name}</b></span></td><td>{contact.company || <span className="sub">Independent contact</span>}</td><td className="sub">{contact.email || "-"}</td><td className="sub num">{contact.phone || "-"}</td><td><span className="cellchip c-mut">{sourceLabel(contact.source)}</span></td><td className="sub num">{displayDate(contact.updated_at)}</td><td onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}><span className="contactTableActions">{pipelineUnavailable ? <span className="sub">Not enabled</span> : <button type="button" className="btn sm" onClick={() => setQuickAddSeed({ contact_id: contact.id, name: contact.name, dealer_name: contact.company, email: contact.email, phone: contact.phone })}>Add to pipeline</button>}{contact.can_archive && <button type="button" className="iconBtn danger" aria-label={`Delete ${contact.name}`} title={`Delete ${contact.name}`} onClick={() => setArchiveTarget(contact)}><Trash2 size={16} /></button>}</span></td></tr>; })}
    ...
    {archiveTarget && <ContactArchiveDialog contact={archiveTarget} onClose={() => setArchiveTarget(null)} onArchived={() => { setArchiveTarget(null); void qc.invalidateQueries({ queryKey: ["crm-contacts"] }); void qc.invalidateQueries({ queryKey: ["dealer-prospects"] }); }} />}
