...
export default function ContactDetailPage() {
  ...
  const openTrackedEmail = () => {
    ...
    if (prospect) router.push(`/marketing/prospects/${prospect.id}?compose=email`);
    else setAddProspect(true);
  };
  const openProspect = (created: DealerProspect) => router.push(`/marketing/prospects/${created.id}?compose=email`);
  return <div className="contactDetail">
    <header className="contactHero"><Link href="/marketing" className="backLink">← Marketing</Link><div className="contactIdentity"><div className="contactAvatar large">{row.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</div><div><span className="eyebrow">Marketing contact</span><h2>{row.name}</h2><p>{row.company || "Independent contact"}</p></div></div><div className="contactQuick"><button type="button" className="btn" disabled={!row.email || matchingProspects.isLoading || matchingProspects.isError} onClick={openTrackedEmail}>{matchingProspects.isLoading ? "Checking…" : matchingProspects.isError ? "Email lookup unavailable" : prospect ? "Email prospect" : "Add prospect & email"}</button><a className="btn pri" href={row.phone ? `tel:${row.phone}` : undefined} aria-disabled={!row.phone}>Call</a></div></header>
    ...
    {addProspect && <ProspectQuickAddModal initialValues={{ contact_id: row.id, name: row.name, dealer_name: row.company, email: row.email, phone: row.phone }} onClose={() => setAddProspect(false)} onCreated={openProspect} onOpenExisting={(prospectId) => router.push(`/marketing/prospects/${prospectId}?compose=email`)} />}
  </div>;
}
