...
import { Callout, Derived, Field, PChip, PPanel, Picks, type StepCtx } from "../ui";

export function Step4Advance({ ctx }: { ctx: StepCtx }) {
  const { draft, prov, computed, saving, readOnly, set } = ctx;
  ...
  return (
    <>
      <div className="pp-cols">
        <PPanel title="The request" sub="What the dealer is asking for and how the advance is sized.">
          <div className="pp-grid">
            <Field ctx={ctx} k="requested" />
            <Field ctx={ctx} k="min_activation" />
            <Field ctx={ctx} k="facility_type" />
            <Field ctx={ctx} k="term" />
            <Field ctx={ctx} k="dealer_cof" />
            <Field ctx={ctx} k="exclusivity" />
            <Field ctx={ctx} k="debt_service" />
            <Field ctx={ctx} k="markup" />
          </div>
          <div className="pp-row" style={{ marginTop: 12 }}>
            <Picks options={SIZING_MODES} value={sizing} onChange={(k) => set("sizing", k)} disabled={readOnly} />
          </div>
