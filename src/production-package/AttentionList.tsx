...
export function AttentionList({ items, onJump }: { items: AttentionItem[]; onJump: (item: AttentionItem) => void }) {
  const groups = STEPS.map((s) => ({ step: s, items: items.filter((i) => i.step === s.key) })).filter((g) => g.items.length);
  return (
    <section className="pp-att" aria-label="Needs attention">
      ...
      {groups.map((g) => (
        <div key={g.step.key} className="pp-att-g">
          <div className="pp-att-step">{g.step.label}</div>
          {g.items.map((item) => (
